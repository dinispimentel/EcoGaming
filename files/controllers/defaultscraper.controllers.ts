...
function _deal_with_request(error: Error, response: RResponse, body: any, res: EResponse) {
    try {
...
type CallbackFunction =
  (error: Error, response: RResponse, body: any, res: EResponse) => void;
function request_with_method(urlQueryParametrized: string, method: string, body: any, res: EResponse, cb: CallbackFunction) {
    let normalized_callback = (error: Error, response: RResponse, body: any) => {cb(error, response, body, res)};
...
export function fetch_scraper(req: ERequest, res: EResponse, scraper: number, path: string, method: string, query_params?: Array<string>, req_body?: object) {
    /**
    ...
    switch (scraper) {
        case 0:
            request_with_method(url_make(G2GScraper.port, path), method, req_body, res, _deal_with_request);
            break;
        case 1:
            request_with_method(url_make(DMSMScraper.port, path), method, req_body, res, _deal_with_request);
            break
        default:
